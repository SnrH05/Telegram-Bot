...
if not TOKEN or not GEMINI_KEY or not KANAL_ID:
    ...
    # sys.exit(1) # Hata almamak için geçici olarak yorum satırı yapıldı, env girilince açabilirsin.

# Gemini Client (Thread içinde çağıracağız)
# API Key yoksa hata vermemesi için try-except bloğu eklenebilir veya env kontrolü yapılmalı.
try:
